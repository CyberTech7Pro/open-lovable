  ...
  try {
    ...
    if (model.startsWith('anthropic/')) {
      ...
      if (m.includes('gpt-oss')) {
        // roteia "gpt-oss" para um modelo real da Groq
        aiModel = groq('llama-3.1-70b-versatile');
      } else {
        aiModel = openai(m);
      }
    } else if (model.startsWith('google/')) {
      // usa o provider correto do Google para retornar um LanguageModel
      aiModel = google(model.replace('google/', ''));
    } else {
      // Default: usar Groq; se vier um nome desconhecido, escolha um modelo seguro
